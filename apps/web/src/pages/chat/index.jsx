...
import {
  useWebsocket,
  WebsocketProvider,
} from "@repo/connection/context/Websocket";
import { useEffect, useState, useRef } from "react";
import { useAuth } from "../../hooks/useAuth";
import { apiFetch } from "@repo/connection/utils/api";
import {
  encryptMessageWithHybridKey,
...
const ChatHeader = ({ chatId, markReady, auth }) => {
  ...
  const fetchMetadata = async () => {
    ...
    try {
      ...
      if (!jsonData.success)
        throw new Error(jsonData.error || "Failed to fetch metadata");
  ...
  const copyInviteLink = async () => {
    ...
    try {
      const chatHybridKey = await resolveChatHybridKey(
        chatId,
        auth.user.username,
      );
      if (!chatHybridKey) {
      ...
      const inviteTransportKey = Array.from(
        crypto.getRandomValues(new Uint8Array(32)),
      )
        .map((b) => b.toString(16).padStart(2, "0"))
        .join("");

      const encryptedGroupKey = await encryptTextWithPassword(
        chatHybridKey,
        inviteTransportKey,
      );
  ...
  const myRole = chatMetadata?.people?.find(
    (p) => p?.id === auth?.user?.id,
  )?.role;
    ...
    <header className="h-[8vh] px-4 bg-gradient-to-r from-white/5 to-white/[0.02] border-b border-white/10 flex items-center justify-between">
      ...
      <div className="relative">
        <button
          ...
          className="px-3 py-2 rounded-lg bg-white/8 hover:bg-white/15 border border-white/10 text-white/80 text-xs"
        >
          Actions
        </button>
        {menuOpen && (
          <div className="absolute top-12 right-0 w-44 rounded-lg border border-white/10 bg-black/90 p-1 z-30">
            <button
              ...
              className="w-full text-left px-3 py-2 text-sm text-white/80 hover:bg-white/10 rounded"
            >
              Invite
            </button>
            <button
            ...
            <button
              ...
              disabled={myRole === "owner" && chatMetadata.people.length > 1}
            >
              {myRole === "owner" && chatMetadata.people.length > 1
                ? "Leave (owner blocked)"
                : "Leave"}
            </button>
          </div>
...
const Sidebar = ({ auth, markReady, logout }) => {
  ...
  const fetchChats = async () => {
    try {
      ...
      if (!jsonData.success)
        throw new Error(jsonData.error || "Failed to fetch chats");
      ...
      toast.error(
        "We couldn't load your chats. See the console for more details.",
      );
    }
  };
          ...
          chats.map((chat) => (
            ...
            >
              <div className="relative flex-shrink-0">
                <img
                  src={chat.image ? chat.image : "/2.png"}
...
const ChatFooter = ({ cId, disabled }) => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const hybridKey = await resolveChatHybridKey(
        cId,
        auth?.user?.username || "",
      );
      if (hybridKey && messageText.trim()) {
        encryptedBody = await encryptMessageWithHybridKey(
          messageText,
          hybridKey,
        );
      }
      ...
      console.error(
        "Could not encrypt outgoing message, sending plaintext.",
        err,
      );
    }
        ...
        <div className="flex gap-2 px-3 py-1.5 bg-white/5 border-t border-white/10 overflow-x-auto custom-scrollbar">
          {attachments.map((file, i) => {
            ...
            const getPreviewIcon = (name) => {
              ...
              if (/\.(js|jsx|ts|tsx|py|html|css|json)$/i.test(name))
                return "💻";
            ...
            return (
              <div
                key={i}
                className="relative flex items-center gap-1.5 flex-shrink-0 group p-1.5 bg-white/5 rounded-md border border-white/10 hover:border-white/20 transition-colors"
              >
                <div className="relative flex-shrink-0">
    ...
    <WebsocketProvider>
      <div className="flex flex-row h-screen w-screen">
        <Sidebar
          auth={auth}
          logout={logout}
          markReady={() => setReadyStates((prev) => ({ ...prev, chats: true }))}
        />
        ...
        <div className="chatUI">
          {!hasChildRoute && (
            <ChatHeader
              ...
              auth={auth}
              markReady={() =>
                setReadyStates((prev) => ({ ...prev, header: true }))
              }
            />
          )}
