  ...
  return (
    <div className="space-y-4 flex flex-col items-center text-center">
      ...
      <p className="subtitle">
        Input exactly your username, then click <strong>Login</strong> down
        below.
      </p>
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      for (const l of lines) {
        if (!l.startsWith(";")) {
          continue; /* Only process lines with ; at the start, which indicates metadata */
        }
        ...
        switch (key.toLowerCase()) {
          case "u":
            ...
            break;
          case "priv":
            data.privateKey = decompress(value);
            break;
          case "d":
            data.expeditedOnDomain = value;
            break;
          default:
            break;
        }
...
const LoginWithFile = ({ credentials, setCredentials }) => {
  ...
  const handleLogin = (e) => {
    ...
    const loginFlow = async () => {
      const { username, privateKey, expeditedOnDomain } =
        await obtainDataFromFile(file);
      const currentDomain = window.location.origin;

      // 3. Start login
      if (!username || !privateKey) {
        toast.error(
          "The selected file is missing required information. Please ensure you are uploading the correct file.",
        );
        return;
      }

      if (expeditedOnDomain && expeditedOnDomain !== currentDomain) {
        toast.loading(`Loading key from a file with different domain...`, {
          duration: 4000,
        });
  ...
  return (
    <div className="space-y-4 flex flex-col items-center text-center">
      <p className="subtitle">
        Select your private key file, then click <strong>Login</strong> down
        below.
      </p>
  ...
  return (
    <>
      <div className="flex flex-col gap-4 text-center">
        ...
        <p className="subtitle">
          You may prefer using <strong>one of the following methods:</strong>
        </p>
...
function LoginPage() {
    ...
    username: {
      ...
      finishButton: (
        <Button
          variant="accent"
          width="full"
          onClick={() => methods[method].submitFn()}
          disabled={loading}
        >
  ...
  return (
    ...
    >
      <div
        className="button-group stack-horizontal"
        style={{ marginBottom: "24px" }}
      >
        <Button variant="ghost" size="small" onClick={() => setMethod("none")}>
          <a href="/">Back</a>
        </Button>
      </div>
